...
public interface EnrollmentsRepository extends JpaRepository<Enrollment, Long> {
    ...
    @Query("SELECT FUNCTION('DATE', e.enrollmentDate), SUM(e.price) FROM Enrollment e WHERE e.enrollmentDate BETWEEN :startDate AND :endDate GROUP BY FUNCTION('DATE', e.enrollmentDate) ORDER BY FUNCTION('DATE', e.enrollmentDate)")
    ...
    // Count distinct users (students) in enrollments
