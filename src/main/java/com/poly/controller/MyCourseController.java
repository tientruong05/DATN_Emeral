...
public class MyCourseController {
    ...
    public String showMyCourses(
            ...
            Model model) { // Removed HttpSession session parameter

        // --- Start of Spring Security integration ---
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (authentication == null || !authentication.isAuthenticated() || !(authentication.getPrincipal() instanceof UserDetails)) {
            log.warn("Người dùng chưa đăng nhập hoặc chưa được xác thực, chuyển hướng đến trang đăng nhập");
        ...
        if (!userOpt.isPresent()) {
            log.error("Không tìm thấy người dùng trong DB với email đã xác thực: {}", userEmail);
            return "redirect:/Login_Signin"; // Should ideally not happen if login worked
        }
        ...
        try {
            ...
            switch (sort) {
                case "progress":
                    ...
                    break;
                case "name":
                    // Note: If 'course.ten_khoa_hoc' is a nested property, ensure your JPA repository can handle this sort.
                    // Sometimes, you might need a custom query or a DTO for complex sorting.
                    sortOption = Sort.by(Sort.Direction.ASC, "course.tenKhoaHoc"); // Use camelCase for property name
                    break;
                case "completion":
                    sortOption = Sort.by(Sort.Direction.DESC, "diem");
                    break;
    ...
    public String searchMyCourses(
        ...
        try {
            ...
            Page<Enrollment> enrollmentPage = enrollmentService.findByUserAndCourseNameContaining(user, query,
                    pageable);
            ...
            log.info(
                    "Đã tải trang tìm kiếm my-courses cho người dùng {} với query '{}', sort '{}', trang {}, kích thước {}",
